function [xn, n_decomp] = ASK_Full_2D(dynamics, p, n, tspan, N, x0, r, fraction, option)
...
% This implemetation is based on two-dimensional systems, and solves for
...
[K, V, L] = approximate_Koopman_2D(N, x_GL, Ds, dynamics, p);

% compute projection coefficients
C = compute_coef_2D(x_GL, V);

% sovler
xc = zeros(2, n+1);           % solutions at check points
xc = x0;
xn = zeros(2, length(tspan)); % solutions at time mesh points
xn(:,1) = x0;
n_decomp = 1;
...
for j = 2:n
    % numerical solver
    [x1, x2] = numerical_solve_2D(t(j), t(t_ind), V, L, C);
    xc(:, j) = [x1, x2];
...
%                 [sol1, sol2] = numerical_solve_2D(tspan(i), t(t_ind), V, L, C);
%                 xn(:, i) = [sol1, sol2];
%             end
...
%         end
        ...
        if ~isempty(tspan_ind_new)
            ...
            [sol1, sol2] = numerical_solve_2D(ct, t(t_ind), V, L, C);
            xn(:, tspan_ind+1:tspan_ind_new) = [sol1; sol2];
...
if tspan_ind < length(tspan)
    ...
    [sol1, sol2] = numerical_solve_2D(ct, t(t_ind), V, L, C);
    xn(:, tspan_ind+1:length(tspan)) = [sol1; sol2];
end
